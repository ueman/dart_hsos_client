...
void main() {
  ...
  test('test download of files', () async {
    ...
    try {
      var courseFiles = await FileArea.getListOfAllFilesForCourse(client, '370183926232552');
      print(courseFiles);
    } finally {
      client.close();
    }
  ...
  test('test fetching of announcements', () async {
    var client = OscaWebClient(fedAuthCookieValue);
    try {
      var announcements = await Announcements.getAllAnnouncementsForCourse(client, '370184054259614');
    } finally {
      client.close();
    }
  });
